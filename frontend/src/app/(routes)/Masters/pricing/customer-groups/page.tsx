...
import { CustomerGroup } from '@/app/types/pricing';
...
export default function CustomerGroupsPage() {
  ...
  const { data: customerGroupsData, isLoading } = useFetchCustomerGroups();
  ...
  const confirmDeleteAction = () => {
    if (confirmDelete.id) {
      deleteCustomerGroup(confirmDelete.id, {
        onSuccess: () => {
          toast.success(t('pricing.customerGroup.deleteSuccess'));
          setConfirmDelete({ open: false, id: null });
        },
        onError: (error) => {
          console.error('Error deleting customer group:', error);
          toast.error(t('pricing.customerGroup.deleteError'));
          setConfirmDelete({ open: false, id: null });
        }
      });
    }
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'name', headerName: t('pricing.customerGroup.name'), flex: 1 },
    { field: 'code', headerName: t('pricing.customerGroup.code'), width: 150 },
    { 
      field: 'is_active', 
      headerName: t('common.status'), 
      width: 120,
      renderCell: (params: GridRenderCellParams<CustomerGroup>) => (
        <Box sx={{ 
    ...
    <Container maxWidth="lg">
      <Box sx={{ mb: 4 }}>
        ...
        <Breadcrumbs aria-label="breadcrumb">
          ...
          <Typography color="textPrimary">{t('pricing.customerGroups')}</Typography>
        </Breadcrumbs>
      </Box>
      ...
      <ConfirmDialog
        ...
        title={t('pricing.customerGroup.confirmDelete')}
        content={t('pricing.customerGroup.confirmDeleteMessage')}
